...
const createAdmin = async (req, res) => {
    try {
        ...
        res.status(200).json({ 
            message: isResend ? 'Admin invitation resent successfully' : 'Admin invited successfully',
            email: email
        });
    } catch (error) {
        res.status(500).json({ 
...
const getPendingAdminInvitations = async (req, res) => {
    try {
        ...
        const pendingAdmins = await User.find({
            role: 'ADMIN',
            invitation: 'pending'
        }).select('-password -mfa_secret -webauthn_credentials').sort({ createdAt: -1 });
...
const acceptPendingAdminInvitation = async (req, res) => {
    try {
        ...
        if (pendingAdmin.invitation === 'pending') {
            if (!pendingAdmin.inviteToken || !pendingAdmin.inviteTokenExpiry || pendingAdmin.inviteTokenExpiry < new Date()) {
                return res.status(400).json({ 
...
const getPendingAdminInvitationById = async (req, res) => {
    try {
        ...
        const invitedBy = await User.findById(pendingAdmin.invitedBy)
