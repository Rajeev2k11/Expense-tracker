...
const getExpensesByCategory = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to get expenses by category', error: error.message });
    }
}
...
const getExpensesByUser = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to get expenses by user', error: error.message });
    }
}
...
const getExpensesByTeam = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to get expenses by team', error: error.message });
    }
}
...
const updateExpense = async (req, res) => {
    try {
        const { amount, date, category, description, createdBy } = req.body;
        const expense = await Expense.findByIdAndUpdate(req.params.id, { amount, date, category, description, createdBy }, { new: true });
        res.status(200).json({ message: 'Expense updated successfully', expense: expense });
    } catch (error) {
...
module.exports = { createExpense, getExpenseById, getAllExpenses, updateExpense, deleteExpense, getExpensesByCategory, getExpensesByUser, getExpensesByTeam }