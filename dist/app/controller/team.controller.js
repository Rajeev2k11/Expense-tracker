...
const frontendAppUrl =
  process.env.FRONTEND_APP_URL?.trim() || "http://localhost:5173";
...
const createTeam = async (req, res) => {
  try {
    ...
    if (!Array.isArray(members)) {
      return res
        .status(400)
        .json({ message: "Members must be an array when provided" });
    }
...
const updateTeam = async (req, res) => {
  try {
    const { name, description, team_leader, members, monthly_budget } =
      req.body || {};
    const updatePayload = {};
    ...
    const team = await Team.findByIdAndUpdate(req.params.id, updatePayload, {
      new: true,
    })
      .populate("team_leader", "name username email role")
      .populate("members", "name username email role");
...
const inviteTeamMembers = async (req, res) => {
  try {
    ...
    if (leaderId !== managerId && !isAdminRole(manager.role)) {
      return res
        .status(403)
        .json({ message: "Only the team manager can invite members" });
    }
    ...
    const existingMemberIds = new Set(
      team.members.map((memberId) => memberId.toString()),
    );
    ...
    for (const userId of uniqueUserIds) {
      ...
      const isAlreadyMember =
        existingMemberIds.has(user._id.toString()) ||
        leaderId === user._id.toString();

      if (isAlreadyMember) {
        skipped.push({
          userId: user._id.toString(),
          reason: "User already part of the team",
        });
        continue;
      }
    ...
    return res
      .status(500)
      .json({ message: "Failed to invite team members", error: error.message });
  }
};
...
const removeTeamMember = async (req, res) => {
  try {
    ...
    const requester =
      await User.findById(requesterId).select("role name email");
    ...
    if (!isRequesterAdmin && !isRequesterManager) {
      return res
        .status(403)
        .json({ message: "Only admin or the team manager can remove members" });
    }
    ...
    const member = await User.findById(memberId).select(
      "member_type role default_team active_team",
    );
    ...
    if (!isRequesterAdmin) {
      if (member.member_type === "MANAGER") {
        return res
          .status(403)
          .json({ message: "Managers can only be removed by an admin" });
      }
    ...
    const filteredMembers = teamMembersArray.filter(
    ...
    if (!removedFromMembers && memberId !== leaderId) {
      return res
        .status(404)
        .json({ message: "Member is not part of the team" });
    }
...
const updateTeamMemberRole = async (req, res) => {
  try {
    ...
    if (!["MANAGER", "MEMBER"].includes(normalizedRole)) {
      return res
        .status(400)
        .json({ message: "Role must be either MANAGER or MEMBER" });
    }
    ...
    if (leaderId !== requesterId) {
      return res
        .status(403)
        .json({ message: "Only the team manager can change member roles" });
    }
    ...
    const isMemberOfTeam =
      teamMemberIds.includes(memberId) || memberId === leaderId;
...
const deleteTeam = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Team deleted successfully", deletedTeam: team });
  } catch (error) {
    res
      .status(500)
